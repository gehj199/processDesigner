...
define(function(require, exports, module) {
  ...
  var AppendLineCommand = kity.createClass('AppendLineCommand', {
    ...
    execute: function(designer, line) {
      ...
      var actionId = line.getData('actionId'),
        condition;
      if(!actionId && line.startNode.data.nodeId.indexOf('userTask')!=-1){
        var formChild=line.startNode.data.form.children;
        var formAction,approveChild;
        if(formChild){
          formChild.forEach(function(child){
            if(child.name=='formActionSet')
              formAction=child;
          })
        }
        if(formAction){
          formAction.children[0].children.forEach(function(child){
            if(child.attributes[2].value.toLowerCase()=='approve')
              approveChild=child;
          })
        }
        actionId=approveChild.attributes[0].value;
      }
      var property;
      if (actionId) {
        condition = 'sys_lastAction = "' + actionId + '"';
      }
      if(line.type=='dotline')
        property = designer.getDotlineDefaultProps();
      else
        property = designer.getLineDefaultProps(condition);
  ...
  var Liner = kity.createClass('Liner', {
    ...
    draw: function(pos, endNode) {
      ...
      this._drawPos = pos;
    //  this._designer.updateDotline(this._node, this._line);
      if (!this._drawMode) {
        // 判断拖放模式是否该启动
      ...
      if (endNode) {
        ...
        var endName=endNode.data.property.name;
        if((endName=='textAnnotation' && this._line.type=='line')||endName=='boundaryEvent'||endName=='startEvent'){
          this._designer.removeLine(this._line);
        }else {
          if(!this._line.getContainer().container)
            this._designer.addLine(this._line);
        }
      } else {
        this._line.endPos = pos;
        this._line.endVector = null;
      }
      var startProperty=this._line.startNode.data.property;
      if(startProperty.name=='boundaryEvent' && startProperty.children[0].name=="compensateEventDefinition"){
        if(this._line.type&& this._line.type=="line"){
          this._line.resetContainers();
          designer.getLineContainer().addShape(this._line.getContainer());
        }
        this._designer.updateDotline(this._node, this._line);
      }
      else if(startProperty.name=='textAnnotation' || (startProperty.name=='endEvent' && this._line.type=='line')){
          this._designer.removeLine(this._line);
      }
      else{
      ...
      if (this._line) {
        ...
        if (endNode && this._node.getData('id') !== endNode.getData(
          ...
          this._designer.executeCommand('AppendLine', this._line,
            true);
          if(this._line.startNode.data.property.name=='boundaryEvent' &&this._line.startNode.data.property.children[0].name=="compensateEventDefinition"){
            if(this._line.endNode.data.property.name=='restServiceTask'){
              var isForCompensation=utils.findProperty(this._line.endNode.data.property.attributes,'isForCompensation');
              isForCompensation.value="true";
              isForCompensation.editor.disabled="true";
            }
            else if(this._line.endNode.data.property.name=='scriptTask'){
              var isForCompensation=utils.findProperty(this._line.endNode.data.property.attributes,'isForCompensation');
              isForCompensation.value="true";
              isForCompensation.editor.disabled="true";
            }
