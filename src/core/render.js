...
define(function(require, exports, module) {
  ...
  function createDesignerExtention() {
    ...
    function renderNode(node, rendererClasses) {
      ...
      if(node && node.processChild && node.processChild.length!=0){
        var offsets=[],maxHeight = 0;
        node.processChild.forEach(function(child){
          renderNode(child, rendererClasses);
          if(maxHeight < child._contentBox.height){
            maxHeight = child._contentBox.height;
          }
          offsets.push(child.getLayoutOffset());
        });
        var offset,childBox,width,height;
        offsets.forEach(function(child){
          if(!offset || offset.x>child.x)
            offset=child;
          if(!height || height.y<child.y)
            height=child
        });
        maxHeight /= 2;
        node.layout({x: 0, y: maxHeight + node._contentBox.bottom});
        node.processChild.forEach(function(child){
          childBox = child.getContentBox();
          width=child.getLayoutOffset()
          node._contentBox = node._contentBox.merge(
            new kity.Box(
              width.x,
              width.y+70,
              childBox.width,
              height.y-offset.y+100
            //  node._contentBox.height
            )
          );
          node.setLayoutOffset({x:offset.x,y:offset.y});
        });
        node._contentBox = node._contentBox.expand(0, 0, maxHeight - node.getStyle('padding-bottom'), 0);
      }
