    ...
    exportJson: function () {
      ...
      children.forEach(function (child, i) {
        if (child.name === 'ioSpecification') {
          ...
          var form=utils.findProperty(child.children,"form");
          if(form){
            formElement.beforeExport(form);
          }
        }
      });
      ...
      children.forEach(function(child){
        if(child && child.children){
          var nodeExtension=utils.findProperty(child.children,'extensionElements');
          if(nodeExtension && nodeExtension.children){
            var nodeForm=utils.findProperty(nodeExtension.children,'form');
            if(nodeForm)
              formElement.beforeExport(nodeForm);
          }
        }
      })
    ...
    importJson: function (json, params) {
      ...
      if(processes && processes.length){
        ...
        for(var i=0;i<proc.children.length;i++){
          if(proc.children[i].name=='userTask')
            ...
            }else if(proc.children[i].name=='extensionElements'){
              var form=utils.findProperty(proc.children[i].children,"form");
              if(form){
                formElement.beforeImport(form);
              }
            }
        }
        ...
        for(var i=0;i<itemDefinitions.length;i++){
          if(itemDefinitions[i].attributes)
      ...
      if (proc) {
        ...
        procChildren.forEach(function (child, i) {
          if (child.name === 'sequenceFlow' || child.name === 'association') {
            lines.push(child);
      ...
      function importNode(designer, jsonNode, nodesDic, lang) {
        ...
        if(jsonNode.name === 'textAnnotation'){
          opts.linePointsFilter = 8; // 对连线点进行限制 1000
        }
